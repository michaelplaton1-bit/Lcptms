...
function unwrapArray(data){
  ...
  if(typeof data==="string"&&/^[\[{]/.test(data.trim())){
    try{return unwrapArray(JSON.parse(data));}catch{return [];}
  }
  ...
  for(const c of candidates){
    ...
    if(typeof c==="string"&&/^[\[{]/.test(c.trim())){
      const nested=unwrapArray(c);
      if(nested.length)return nested;
    }
    if(c && typeof c==="object"){
      for(const nested of Object.values(c)){
        if(Array.isArray(nested)) return nested;
        const deeper=unwrapArray(nested);
  ...
  for(const value of Object.values(data)){
    const nested=unwrapArray(value);
    if(nested.length)return nested;
  }
