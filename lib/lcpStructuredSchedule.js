...
// v1.7.2: official LCP LC AI Current Set for validation.
// The native site calls GetTideSetForDateAndDays with start + days.
// Keep the raw payload because field names may change and the validator
// deliberately parses defensively.
export async function fetchOfficialCurrentSet({days=14}={}){
  const jar=await authenticate();
  const result=await apiGet(jar,ENDPOINTS.tideSet,{
    start:new Date().toUTCString(),
    days
  });
  if(!result.ok) throw new Error(`Official current set failed: ${result.status}`);
  return {
    fetchedAt:new Date().toISOString(),
    data:result.data,
    diagnostics:{
