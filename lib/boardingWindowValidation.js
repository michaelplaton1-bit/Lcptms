...
function categoryFromText(s){
  const x=norm(s);
  if(x.includes("INBOUND")&&x.includes("DEEP"))return "INBOUND_DEEP_DRAFT";
  if(x.includes("OUTBOUND")&&x.includes("CAMERON")&&(x.includes("UP TO 38")||x.includes("<=38")||x.includes("LESS THAN 38")))return "CLNG_OUTBOUND_LE_38";
  if(x.includes("OUTBOUND")&&x.includes("CAMERON")&&(x.includes("38' TO 40")||x.includes("GREATER THAN 38")||x.includes(">38")))return "CLNG_OUTBOUND_GT_38";
...
export function parseOfficialWindows(payload){
  ...
  for(const obj of valuesDeep(payload)){
    const label=first(obj,["Category","Title","Name","Type","WindowName","TideSetName","Description","Header"]);
    const category=categoryFromText(label);
    if(!category)continue;
    const open=first(obj,["Open","OPEN","OpenTime","WindowOpen","Start"]);
    const close=first(obj,["Close","CLOSE","CloseTime","WindowClose","End"]);
    if(open&&close){
      const oi=chicagoIsoFromOfficial(open),ci=chicagoIsoFromOfficial(close);
      if(oi&&ci)records.push({category,label:String(label),open:oi,close:ci,native:obj});
    }
  }
  return records;
}
