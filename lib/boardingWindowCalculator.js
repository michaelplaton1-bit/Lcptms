...
// This is the sole active LCPTMS boarding-window prediction source.
...
function interpolateAt(points,iso,key){
  if(!iso||!Array.isArray(points)||points.length<1)return null;
  const t=new Date(iso).getTime();
  if(!Number.isFinite(t))return null;
  const rows=[...points].filter(p=>p?.isoTime&&Number.isFinite(Number(p[key]))).sort((a,b)=>new Date(a.isoTime)-new Date(b.isoTime));
  ...
  for(let i=1;i<rows.length;i++){
    ...
    if(tb===ta)return {...a,interpolated:true,requestedIsoTime:iso};
    const f=(t-ta)/(tb-ta);
    return {
      isoTime:iso,
      [key]:Number(a[key])+f*(Number(b[key])-Number(a[key])),
      interpolated:true,
      bracket:[a.isoTime,b.isoTime]
    };
  }

  // Only allow endpoint fallback if observation is within 20 min.
  const nearest=rows.reduce((best,p)=>{
    const d=Math.abs(new Date(p.isoTime).getTime()-t);
    return !best||d<best.d?{p,d}:best;
  },null);
  return nearest&&nearest.d<=20*MIN?{...nearest.p,interpolated:false,requestedIsoTime:iso}:null;
}
...
export async function calculateBoardingWindows(){
  ...
  const predictedAtActual=interpolateAt(prediction,currentActual?.isoTime,"velocityKt");
  const currentResidualKt=(currentActual&&predictedAtActual)
    ? currentActual.velocityKt-predictedAtActual.velocityKt:null;

  const tidePredAtActual=interpolateAt(tidePrediction,tideActual?.isoTime,"heightFt");
