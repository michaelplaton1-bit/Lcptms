...
export async function writeOfficialWindowCapture(capture){
  if(!learningStoreConfigured())throw new Error("Learning storage or encryption secret is not configured");
  const d=new Date(capture.capturedAt||Date.now());
  const yyyy=d.getUTCFullYear(),mm=String(d.getUTCMonth()+1).padStart(2,"0"),dd=String(d.getUTCDate()).padStart(2,"0");
  const half=String(d.getUTCHours()<12?0:12).padStart(2,"0");
  const bucketDate=new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),d.getUTCDate(),Number(half)));
  const path=`${ROOT}/official-windows/${yyyy}/${mm}/${dd}/${safeStamp(bucketDate)}.json`;
  await writeEncrypted(path,capture,{allowOverwrite:true});
  await writeEncrypted(`${ROOT}/official-windows/latest.json`,capture,{allowOverwrite:true});
  return path;
}

export async function readLatestOfficialWindowCapture(){
  if(!learningStoreConfigured())return null;
  try{return await readStored(`${ROOT}/official-windows/latest.json`);}
  catch{return null;}
}
